...
const Flag = React.memo(({ flag, size }: FlagProps) => {
	...
	return (
		<Box as="span" className={`self-center flex-shrink-0 inline-flex items-center ${size ? `w-${size} h-${size}` : 'w-6 h-6'}`} aria-label={altText}>
			<img src={imgSrc} alt={altText} style={{ width: '100%', height: '100%', objectFit: 'contain' }} loading="lazy" />
		</Box>
	)
})
